...
import { NewJobModal } from './components/NewJobModal.tsx';
import { ConfirmationModal } from './components/ConfirmationModal.tsx';
import { PlusIcon, SearchIcon, FilterIcon, FlagIcon } from './components/Icons.tsx';
import { LoginScreen } from './components/LoginScreen.tsx';
import { AdminPanel } from './components/AdminPanel.tsx';
import { MetricsPanel } from './components/MetricsPanel.tsx';
import { Toast } from './components/Toast.tsx';
import type { Job, User, JobStatus, ToastType, JobFlag, Note, ConfirmationState, Action, ActivityLog } from './types.ts';
import { INITIAL_JOBS, USERS, ALL_STATUSES, ALL_FLAGS, getNextAction } from './constants.ts';
...
export default function App() {
  ...
  useEffect(() => {
    const fetchAndSeedData = async () => {
      ...
      if (existingJobs && existingJobs.length === 0) {
        const jobsToInsert = INITIAL_JOBS.map(({id, ...rest}) => ({
            ...rest,
            creationDate: rest.creationDate.toISOString(),
            history: rest.history.map(h => ({ ...h, timestamp: h.timestamp.toISOString() })),
            notes: rest.notes.map(n => ({ ...n, timestamp: n.timestamp.toISOString() })),
        }));
        const { data: seededJobs, error: seedError } = await supabase.from('jobs').insert(jobsToInsert).select();
  ...
  const handleAddJob = async (newJobData: { orderNumber: string; note: string; }) => {
    ...
    const jobToInsert = {
      ...
      isArchived: false,
      notes: notes.map(n => ({ ...n, timestamp: n.timestamp.toISOString() })),
  ...
  const updateJobStatus = async (jobIds: string[], newStatus: JobStatus) => {
    ...
    const updates = jobsToUpdate.map(job => {
        ...
        return supabase.from('jobs').update({ status: newStatus, history: newHistory.map(h => ({ ...h, timestamp: h.timestamp.toISOString() })) }).eq('id', job.id);
    });
  ...
  const updateJobFlag = async (jobId: string, newFlag: JobFlag) => {
    ...
    const { error } = await supabase.from('jobs').update({ flag: newFlag, history: newHistory.map(h => ({ ...h, timestamp: h.timestamp.toISOString() })) }).eq('id', jobId);
  ...
  const handleArchiveJobs = async (jobIds: string[]) => {
      ...
      const updates = jobs.filter(j => jobIds.includes(j.id)).map(job => {
        ...
        return supabase.from('jobs').update({ isArchived: true, history: newHistory.map(h => ({...h, timestamp: h.timestamp.toISOString() })) }).eq('id', job.id);
      });
  ...
  const handleRestoreJobs = async (jobIds: string[]) => {
      ...
      const updates = jobs.filter(j => jobIds.includes(j.id)).map(job => {
        ...
        return supabase.from('jobs').update({ isArchived: false, history: newHistory.map(h => ({...h, timestamp: h.timestamp.toISOString() })) }).eq('id', job.id);
      });

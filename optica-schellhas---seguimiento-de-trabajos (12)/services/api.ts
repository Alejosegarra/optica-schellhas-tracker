...
export const apiUpdateJob = async (jobId: string, updates: Omit<Database['public']['Tables']['jobs']['Update'], 'updated_at' | 'history'>, updatedBy: User): Promise<Job> => {
    const { data: currentJob, error: fetchError } = await supabase.from('jobs').select('history, status').eq('id', jobId).single();
...
export const apiBulkUpdateJobs = async (jobIds: string[], status: JobStatus, updatedBy: User): Promise<Job[]> => {
    const { data: currentJobs, error: fetchError } = await supabase.from('jobs').select('*').in('id', jobIds);
    if (fetchError) throw fetchError;
    if (!currentJobs) return [];
    
    const now = new Date().toISOString();
    const jobsToUpdate: Database['public']['Tables']['jobs']['Update'][] = currentJobs.map(job => ({
        ...job,
        status,
        updated_at: now,
...
export const apiDeleteUser = async (id: string): Promise<{ success: true }> => {
    ...
    if(userToDelete.role === Role.Admin){
        const { count } = await supabase.from('users').select('*', { count: 'exact', head: true }).eq('role', Role.Admin);
        if (count === 1) {

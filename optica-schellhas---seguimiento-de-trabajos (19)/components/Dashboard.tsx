...
import { Role, JobStatus, JobPriority, type User, type Job, type Announcement, type JobHistoryEntry } from '../services/database.types.ts';
import * as api from '../services/api.ts';
import { supabase } from '../services/supabaseClient.ts';
import JobCard from './JobCard.tsx';
import { Button, Input, Modal, Card, CardHeader, CardContent, Select } from './common/UI.tsx';
import { EyeIcon, LogOutIcon, UsersIcon, PlusIcon, KeyIcon, TrashIcon, HistoryIcon, SearchIcon, BriefcaseIcon, AlertTriangleIcon, BranchIcon, InboxIcon, ClipboardListIcon, ChevronRightIcon, SendIcon, CheckIcon, CheckCircleIcon, TruckIcon, MegaphoneIcon, XIcon } from './common/Icons.tsx';
        ...
        <Modal isOpen={!!job} onClose={onClose} title={`Historial del Trabajo #${job.id}`} size="lg">
            <div className="space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <ul className="space-y-3">
                    {[...((job.history as unknown as JobHistoryEntry[]) || [])].sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).map((entry, index) => (
...
// --- ADMIN PANEL ---
type AdminView = 'dashboard' | 'accounts' | 'announcements';
    ...
    return (
        <div>
            <div className="flex border-b border-gray-200 mb-6">
                {(['dashboard', 'accounts', 'announcements'] as AdminView[]).map(v => (
                    <button key={v} onClick={() => setView(v)} className={`capitalize py-2 px-4 text-sm md:text-base font-medium ${view === v ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500'}`}>
                        {v === 'dashboard' ? 'Métricas' : v === 'accounts' ? 'Cuentas' : 'Anuncios'}
                    </button>
                ))}
            </div>
...
const BranchView: React.FC<{ user: User }> = ({ user }) => {
    ...
    const { activeJobs, historicalJobs, groupedJobs } = useMemo(() => {
        ...
        return { activeJobs: active, historicalJobs: historical, groupedJobs: grouped };
    }, [jobs]);

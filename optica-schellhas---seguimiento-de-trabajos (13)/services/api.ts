...
import { Role, JobStatus, JobPriority, type User, type Job, type Announcement, type Database } from './database.types.ts';
...
export const apiGetLoginUsers = async (): Promise<Pick<User, 'id' | 'username'>[]> => {
    ...
    return (data as Pick<User, 'id' | 'username'>[]) || [];
};
...
export const apiUpdateJob = async (jobId: string, updates: Omit<Database['public']['Tables']['jobs']['Update'], 'updated_at' | 'history'>, updatedBy: User): Promise<Job> => {
    ...
    const currentJob = currentJobData as Pick<Job, 'history' | 'status'> | null;
...
export const apiBulkUpdateJobs = async (jobIds: string[], status: JobStatus, updatedBy: User): Promise<Job[]> => {
    ...
    const currentJobs = currentJobsData as Job[] | null;
    ...
    const { data, error } = await supabase.from('jobs').upsert(jobsToUpdate as any).select();
...
export const apiGetStats = async (): Promise<{
    ...
    const allJobs = (allJobsData as Pick<Job, 'branch_name' | 'priority'>[]) || [];
...
export const apiUpdateUserPassword = async (id: string, newPassword: string):Promise<User> => {
    const { data, error } = await supabase.from('users').update({ password: newPassword } as any).eq('id', id).select().single();
    if (error) throw error;
...
export const apiDeleteUser = async (id: string): Promise<{ success: true }> => {
    ...
    const userToDelete = userToDeleteData as Pick<User, 'role'> | null;
    if (!userToDelete) {
...
export const apiAddAnnouncement = async (message: string): Promise<Announcement> => {
    const { data, error } = await supabase.from('announcements').insert({ message } as any).select().single();
    if (error) throw error;
